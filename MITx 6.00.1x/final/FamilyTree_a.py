    ...
    def cousin(self, a, b):
        ...
        def getPath(node):
            if node.parent == None:
                ...
                return [node] + getPath(node.parent)
                
        trace_a = getPath(self.names_to_nodes[a])
        trace_b = getPath(self.names_to_nodes[b])

        try:
            while trace_a[-1] == trace_b[-1]:
                trace_a.pop()
                trace_b.pop()
            
            if len(trace_a) < len(trace_b):
                closest = len(trace_a) - 1
            else:
                closest = len(trace_b) - 1
        
        except:
            closest = -1

        return (closest, abs((len(trace_b) - len(trace_a))))
